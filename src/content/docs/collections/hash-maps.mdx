...
Now that you've completed the Collections section and learned about vectors, strings, and hash maps in Rust, you're ready to move on to [Error Handling](/error-handling/). In the next chapter, we'll explore how Rust handles errors compared to JavaScript's exception model. 